...
*/
;(function() {
var CORDOVA_JS_BUILD_LABEL = '2.5.0nodewebkit-21-g549e08a';
...
module.exports = function(success, fail, service, action, args) {
    ...
    var proxy = commandProxy.get(service,action)
      , commandStr = service + "::" + action
      , errorStr;
    
    function safeSuccessCB(payload) {
            ...
            setTimeout(function() {
                try {
                    ...
                    console.error(commandStr +
                                  ": exception in success callbak [" + err + "].");
                }
            }, 0);
        } else if (payload) {
            console.warn(commandStr +
                         ": no proper success callback supplied. Returned payload ignored.");
        }
    }

    function safeFailCB(msg) {
        if (typeof fail === "function") {
            setTimeout(function() {
                try {
                    ...
                    console.error(commandStr +
                                  ": exception in error callbak [" + err + "].");
                }
            }, 0);
        } else {
            console.error(commandStr + " (no proper error callback supplied) [" + msg + "]");
        }
    ...
    if(proxy) {
        ...
        try {
            proxy(safeSuccessCB, safeFailCB, args);
        }
        catch(e) {
            errorStr = "Exception calling native with command " + commandStr + " - exception=" + e;
            safeFailCB(errorStr);
        }
    }
    else {
        errorStr = commandStr + ": not implemented.";
        safeFailCB(errorStr);
    }
};
...
// file: lib/nodewebkit/platform.js
...
/*
 * Define a helper function to find modules sitting
 * in a node_modules at the same level than
 * cordova.js or cordova.nodewebkit.js
 */

// window.require works for 'nw.gui'
// global.require is nodejs-only

cordova.npmRequire = window.require;
cordova.npmRequireResolve = (function(){
    var baseUrl = ''
      , basePath = ''
      , ii
      , pattern = /cordova.js$|cordova.nodewebkit.js$/
    ...
    for (ii=0; ii < scripts.length; ii++) {
        if (pattern.test(scripts[ii].src)) {
            baseUrl = scripts[ii].src;
            break;
        }
    }
    ...
    basePath = path.dirname(path.resolve(basePath));

    basePath = path.join(basePath, 'node_modules');
    baseUrl = baseUrl.substr(0, baseUrl.lastIndexOf('/')) + '/node_modules';

    return function(moduleName, options) {
        var resolvedPath =  path.join(basePath, moduleName);
 ...
 */
modulemapper.loadMatchingModules(/cordova.*\/nodewebkit\/.*Proxy$/);

/*
 * Make the commandProxy available to external modules, so commands can
 * be defined and overriden
 */
cordova.commandProxy = require('cordova/commandProxy');
...
// file: lib/nodewebkit/plugin/nodewebkit/CameraProxy.js
define("cordova/plugin/nodewebkit/CameraProxy", function(require, exports, module) {

var cordova = require('cordova');

module.exports = {

    takePicture:function(win,fail,args) {
        console.log("NativeProxy::takePicture");

        var name = cordova.npmRequireResolve('cordova-miniapps')
          , miniapps = cordova.npmRequire(name)
          , options = {
                width: 400,
                height: 600,
                title: "camera"
          ...
          , params = {
                ...
                unused_popoverOptions: args[10]
            };

        miniapps.open(win, fail, cordova, 'camera', options, params);
    }
...
// file: lib/nodewebkit/plugin/nodewebkit/DeviceProxy.js
define("cordova/plugin/nodewebkit/DeviceProxy", function(require, exports, module) {

/* global CORDOVA_JS_BUILD_LABEL */
...
module.exports = {
    ...
    getDeviceInfo:function(win /*,fail*/ /*,args*/) {
        ...
        var deviceId = utils.createUUID();

        setTimeout(function () {
            win({ platform: "nodewebkit+something", version: "0", name: "name", uuid: deviceId, cordova: CORDOVA_JS_BUILD_LABEL });
        }, 0);
    }
...
// file: lib/nodewebkit/plugin/nodewebkit/FileProxy.js
define("cordova/plugin/nodewebkit/FileProxy", function(require, exports, module) {
...
*/
    requestFileSystem:function(win,fail,args) {
        ...
        window.webkitRequestFileSystem(type, size, win, fail);
...
// file: lib/nodewebkit/plugin/nodewebkit/GlobalizationProxy.js
define("cordova/plugin/nodewebkit/GlobalizationProxy", function(require, exports, module) {

var cordova = require('cordova')
  , nativeExec = cordova.npmRequire(cordova.npmRequireResolve('cordova-exec'))
  , service = nativeExec.service('cordova-core');

module.exports = {

    getLocaleName: function(win,fail,args) {
    ...
    getPreferredLanguage: function(win,fail,args) {
        console.log("Globalization::getPreferredLanguage");
        service.exec(win,
                     fail,
...
require("cordova/commandProxy").add("Globalization",module.exports);

});

// file: lib/nodewebkit/plugin/nodewebkit/NetworkStatusProxy.js
define("cordova/plugin/nodewebkit/NetworkStatusProxy", function(require, exports, module) {
...
function setupNetworkAsyncNotifications() {

    var daemonRunning = false
      , initialConnectionType = network.type
      , TIMEOUT = 120000;
    function daemonWin(networkInfo) {
        daemonRunning = false;
        network.type = networkInfo.connectionType;
        if (network.type === "none" ||
            initialConnectionType === "none") {
            cordova.fireDocumentEvent((network.type === "none")?
                                      "offline":
                                      "online");
        }
        initialConnectionType = network.type;
        startNetworkStatusChangeDaemon();
    }
    function daemonFail(err) {
        var nextStartDelay = 1
          , timedOut = true;

        if (err !== "timed out while waiting for network status change") {
            console.log("NetworkAsyncNotifications daemon error: " + err);
            timedOut = false;
            nextStartDelay = 30000;
        }
        daemonRunning = false;
        setTimeout(startNetworkStatusChangeDaemon, nextStartDelay);
    }
    function startNetworkStatusChangeDaemon() {
        if (!daemonRunning) {
            daemonRunning = true;
            service.exec(daemonWin,
                         daemonFail,
                         'getNewNetworkStatus',
                         [initialConnectionType, TIMEOUT],
                         {verbose: false});
        }
    }
    startNetworkStatusChangeDaemon();
...
document.addEventListener("deviceready",
                          setupNetworkAsyncNotifications,
                          false);
...
module.exports = {
    ...
    getConnectionInfo:function(win,fail,args)
    {
        ...
        service.exec(function(payload) {
                        win(payload.connectionType);
                    },
                     fail,
                     'getNetworkStatus',
                     args,
...
// file: lib/nodewebkit/plugin/nodewebkit/NotificationProxy.js
define("cordova/plugin/nodewebkit/NotificationProxy", function(require, exports, module) {
...
module.exports = {
    ...
    alert: function(win,fail,args) {
        var msg = (args && args.length >= 1)? args[0] : "";

        if (args && args.length >= 2) {
            if (args[1] && args[1] !== "Alert") {
                console.warn("navigator.notification.alert: " +
                             "custom alert title ignored");
            }
            if (args.length >= 3) {
                if (args[2] && args[2] !== "OK") {
                    console.warn("navigator.notification.alert: " +
                                 "custom button label ignored");
                }
            }
        }
    ...
    confirm: function(win,fail,args) {
        var msg = (args && args.length >= 1)? args[0] : "";

        if (args && args.length >= 2) {
            if (args[1] && args[1] !== "Confirm") {
                console.warn("navigator.notification.confirm: " +
                             "custom dialog title ignored");
            }
            if (args.length >= 3) {
                if (args[2] && args[2] !== "OK,Cancel") {
                    console.warn("navigator.notification.confirm: " +
                                 "custom button labels ignored");
                }
            }
        }
    ...
    prompt: function(win,fail,args) {
        var msg = (args && args.length >= 1)? args[0] : "";
        var defTxt = (args && args.length >= 4)? args[3] : "";

        if (args && args.length >= 2) {
            if (args[1] && args[1] !== "Prompt") {
                console.warn("navigator.notification.prompt: " +
                             "custom dialog title ignored");
            }
            if (args.length >= 3) {
                if (args[2] && args[2].join(',') !== "OK,Cancel") {
                    console.warn("navigator.notification.prompt: " +
                                 "custom button labels ignored");
                }
            }
        ...
        setTimeout(function() {
            var txt = window.prompt(msg, defTxt)
              , result = {};
            result.buttonIndex = (txt === null)? 2 : 1;
            result.input1 = txt;
            win(result);
        }, 0);
    },

    beep: function(win, fail, args) {

        var numBeeps = args? args[0] : 1;

        if (!audioContext) {
            audioContext = new window.webkitAudioContext(); // jshint ignore: line
        }
        function beep() {
        ...
        for (var ii=0; ii < numBeeps; ii++) {
            setTimeout(beep, ii * 250);
        }
    }
};
...
// file: lib/scripts/bootstrap-nodewebkit.js
